...
// ─── POST /api/fg-stock/bulk-return-accept ────────────────────────────────
// Accept QC pending items back to available stock.
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const item of items) {
      ...
      // Move from qcPendingQty to availableQty
      ...
      const { error: updErr } = await supabase
        ...
        .update({
          ...
          availableQty: prevAvail + quantity,
      ...
      const { error: mvtErr } = await supabase.from('FGStockMovement').insert({
        ...
        toStatus: 'Available',
        referenceType: 'QC',
        reason: reason || 'Return QC accepted',
        movedBy: 'System',
      })
      if (mvtErr) {
      ...
      processed.push({
        ...
        success: true,
        movedQty: quantity,
        newAvailable: prevAvail + quantity,
    ...
    console.error('[FG-Stock Bulk Return Accept POST]', error)
    return NextResponse.json(
      { error: error.message || 'Bulk return accept failed' },
      { status: 500 },
    )
  }
}
