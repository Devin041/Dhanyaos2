...
export async function POST(req: NextRequest) {
  try {
    ...
    const { salesOrderId, styleNo, styleName, targetQty, endDate, startDate } = body
    if (!styleNo || !styleName || !targetQty) return NextResponse.json({ error: 'styleNo, styleName, and targetQty are required' }, { status: 400 })

    if (salesOrderId) {
      const { data: order } = await supabase.from('SalesOrder').select('id, status').eq('id', salesOrderId).single()
    ...
    const { data: job, error } = await supabase.from('ProductionJob').insert({
      jobNo, salesOrderId: salesOrderId || null, styleNo, styleName,
      targetQty: Number(targetQty), completedQty: 0, stage: 'Fabric Issue', status: 'In Progress',
